...
class SMC100DisabledStateException(Exception):
    def __init__(self, state):
        super(SMC100DisabledStateException, self).__init__(
            "Disabled state encountered: " + state
        )


class SMC100RS232CorruptionException(Exception):
    def __init__(self, c):
        super(SMC100RS232CorruptionException, self).__init__(
            "RS232 corruption detected: %s" % (hex(ord(c)))
        )
...
class SMC100(object):
    ...
    def __init__(
        self, smcID, port, backlash_compensation=True, silent=True, sleepfunc=None
    ):
        """
